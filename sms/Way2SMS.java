...
public class Way2SMS {
	...
	public String loginWay2SMS(String userName, String password) {
		...
		if (cookie == null || cookie.isEmpty()) {
			System.err
					.println("Some error occured...Try again in a few seconds..If still problem exists check your username and password");
		}
    ...
    {  
        final String USERNAME="8892653731";//REQUIRED
        final String PASSWORD="rawat";//REQUIRED
        final String ACTION="5CA2C92EAB288472759CE166679AD205.w805";//REQUIRED : In order to understand ACTION value please read the blog  //open Quick SMS page....Right click to view source
        Way2SMS sms=new Way2SMS();
        //HTTP PROXY
        //sms.setProxy("10.1.1.1",8080); //REQUIRED ONLY IF CONNECTING THROUGH A PROXY
        
        StringBuilder phoneNumber= new StringBuilder("9058591840");
        StringBuilder message=new StringBuilder("Welcome to Abhishek JAVA Power");
        /*if(args.length>0)
        {
      ...
      /*  ACTION key.
        ...
        To get Action key use add-on given by Mozilla or the latest IE.

        Download Fire bug for mozilla. by http://getfirebug.com/

        Once u log in in Way2sms......open Quick SMS page....Right click to view source......

        Use find (Ctrl + f )  to find ur unique ACTION key....

        It must be in such format.....

        input type="hidden" name="Action" id="Action" value="asd4kbjfkds"*/

        //http://site23.way2sms.com/main.action?section=s&Token=5CA2C92EAB288472759CE166679AD205.w805&vfType=register_verify
    }
}
