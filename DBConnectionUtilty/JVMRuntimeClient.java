...
import sun.jvmstat.monitor.VmIdentifier;
 ...
 class JVMRuntimeClient {
	public static Map<String, Integer> returning_main(String[] args) throws Exception {

		if (null == args) {
			System.out.println("Usage: java JVMRuntimeClient HOSTS SERVICE RESOURCE");
		}

		Map<String, Integer> openedConBuckets = new HashMap<>();
								...
								for (String rn : resourceNames) {
									...
									try {
										...
										if (args[2].equalsIgnoreCase(rn) || args[2].equalsIgnoreCase("all")) {
											int openedConnections = (Integer) remote.invoke(bean, "findNumOpenConnections",
													new Object[] { rn }, new String[] { String.class.getName() });
											if (openedConnections > 0)
												//	System.out.print(rn+"="+val+",");
												openedConBuckets.put(rn,openedConnections);
										}
									} catch (Exception e) {
